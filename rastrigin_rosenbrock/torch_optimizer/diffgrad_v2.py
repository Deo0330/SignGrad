    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                # mean = torch.mean(abs(grad))
                # std = torch.std(abs(grad))
                # dfc1 = 1. / (1. + torch.exp(-( previous_grad - grad )))
                # dfc2 = 9. / (1. + torch.exp(-( 0.5 * diff )))-4
                # dfc3 = 1. / (1. + torch.exp(-( std * diff -mean)))
                # dfc4 = 1. / (1. + torch.exp(-( std * std *  diff - mean )))
                # dfc5 = 1. / (1. + torch.exp(-( torch.sqrt(std) * diff - mean )))
